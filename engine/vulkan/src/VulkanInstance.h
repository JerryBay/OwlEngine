...
[[nodiscard]] std::optional<InstanceConfiguration> SelectInstanceConfiguration(
    std::uint32_t loaderApiVersion, std::span<const char* const> requiredExtensions,
    std::span<const VkExtensionProperties> availableExtensions,
    std::span<const VkLayerProperties> availableLayers, bool requestValidation, std::string& error);
