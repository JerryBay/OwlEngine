...
[[nodiscard]] std::string VulkanError(const std::string_view operation, const VkResult result)
{
    return std::string{operation} + " failed with VkResult " +
           std::to_string(static_cast<int>(result));
}

[[nodiscard]] bool HasExtension(const std::span<const VkExtensionProperties> availableExtensions,
...
[[nodiscard]] bool HasLayer(const std::span<const VkLayerProperties> availableLayers,
                            const std::string_view requestedName)
{
    return std::ranges::any_of(availableLayers, [requestedName](const auto& layer)
...
void AppendUniqueExtension(std::vector<const char*>& extensions, const char* extension)
{
    ...
    const bool alreadyEnabled =
        std::ranges::any_of(extensions, [requestedName](const char* enabledExtension)
...
[[nodiscard]] bool EnumerateInstanceExtensions(std::vector<VkExtensionProperties>& extensions,
                                               std::string& error)
{
...
namespace detail
{
std::optional<InstanceConfiguration>
SelectInstanceConfiguration(const std::uint32_t loaderApiVersion,
                            const std::span<const char* const> requiredExtensions,
                            const std::span<const VkExtensionProperties> availableExtensions,
                            const std::span<const VkLayerProperties> availableLayers,
                            const bool requestValidation, std::string& error)
{
    error.clear();
    ...
    {
        configuration.validationWarning = "Validation enabled without a debug messenger because "
                                          "VK_EXT_debug_utils is unavailable";
        return configuration;
    }

    configuration.debugMessengerEnabled = true;
    AppendUniqueExtension(configuration.enabledExtensions, VK_EXT_DEBUG_UTILS_EXTENSION_NAME);
    return configuration;
}
                               ...
                               const VkDebugUtilsMessengerCallbackDataEXT* callbackData)
{
    ...
    if (callbackData->objectCount > 0 && callbackData->pObjects != nullptr)
    {
        ...
        for (std::uint32_t index = 0; index < callbackData->objectCount; ++index)
        {
            ...
            stream << "{type=" << static_cast<int>(object.objectType) << ", handle=0x" << std::hex
                   << object.objectHandle << std::dec;
...
std::optional<VulkanInstance> VulkanInstance::Create(std::string& error)
{
    ...
    {
        error =
            "SDL Vulkan Loader is unavailable; create a WindowSurfaceApi::Vulkan window first: " +
    ...
    auto configuration = detail::SelectInstanceConfiguration(loaderApiVersion, requiredExtensions,
                                                             availableExtensions, availableLayers,
