...
namespace owl::vulkan
{
    namespace
    {
                            ...
                            EnumeratedCandidate& candidate, std::string& error)
        {
            ...
            {
                return vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, count,
                                                            extensions);
            };
            ...
            for (std::size_t index = 0; index < queueProperties.size(); ++index)
            {
                ...
                {
                    error = VulkanError("vkGetPhysicalDeviceSurfaceSupportKHR(queueFamily=" +
                                            std::to_string(index) + ")",
                                        presentResult);
                    return false;
                }
            ...
            // VUID-vkGetPhysicalDeviceSurfaceFormatsKHR-surface-06525 requires reported surface
            // support before querying formats; present-mode enumeration has the analogous
            // requirement.
            if (detail::HasReportedSurfaceSupport(candidate.queueFamilies))
            {
                ...
                if (!EnumerateVulkanValues("vkGetPhysicalDeviceSurfaceFormatsKHR",
                                           enumerateSurfaceFormats, candidate.surfaceFormats,
                                           error))
                {
                    return false;
                }
                                          ...
                                          const detail::DeviceEvaluation& evaluation)
        {
            ...
            message << "Selected physical device '" << DeviceName(properties) << "'"
                    ...
                    << ", presentQueueFamily=" << *evaluation.queueFamilies.presentFamily
                    << ", queueMode="
    ...
    namespace detail
    {
        ...
        SelectQueueFamilies(const std::span<const QueueFamilyInfo> queueFamilies) noexcept
        {
            ...
            for (std::size_t index = 0; index < queueFamilies.size(); ++index)
            {
                ...
                if (!selection.graphicsFamily.has_value() &&
                    (family.flags & VK_QUEUE_GRAPHICS_BIT) != 0)
                {
        ...
        bool
        HasReportedSurfaceSupport(const std::span<const QueueFamilyInfo> queueFamilies) noexcept
        {
            return std::ranges::any_of(queueFamilies, [](const QueueFamilyInfo& family)
                                       { return family.presentSupported; });
        }

        std::optional<std::string_view> FindMissingRequiredDeviceExtension(
            const std::span<const VkExtensionProperties> extensions) noexcept
        {
        ...
        SelectSurfaceFormat(const std::span<const VkSurfaceFormatKHR> formats) noexcept
        {
            ...
            const auto srgb = std::ranges::find_if(formats,
                                                   [](const VkSurfaceFormatKHR& format)
                                                   {
                                  ...
                                  std::string& rejectionReason)
        {
            ...
            {
                rejectionReason = "requires Vulkan 1.3; candidate reports " +
                                  ApiVersionString(candidate.apiVersion);
                return std::nullopt;
            }

            if (const auto missingExtension =
                    FindMissingRequiredDeviceExtension(candidate.extensions))
            {
                rejectionReason =
                    "missing required device extension " + std::string{*missingExtension};
                return std::nullopt;
            }
                                                                       ...
                                                                       std::string& error)
    {
        ...
        for (const VkPhysicalDevice physicalDevice : physicalDevices)
        {
            ...
            {
                const std::string rejection = "Physical device '" +
                                              DeviceName(candidate.properties) +
            ...
            error.clear();
            return VulkanDeviceSelection{physicalDevice, candidate.properties,
                                         std::move(*evaluation)};
        }
    ...
    VulkanDeviceSelection::VulkanDeviceSelection(const VkPhysicalDevice physicalDevice,
        ...
        : physicalDevice_(physicalDevice), properties_(properties),
